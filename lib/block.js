...
class Block {
    ...
    async _run( run_context, deps_domain, cancel, params, context ) {
        let h_timeout = null;
        ...
        try {
            result = await ( async () => {
                deps = await this._do_options_deps( run_context, deps_domain, cancel );
                ...
                try {
                    result = await this._action( run_context, deps_domain, cancel, params, context, deps );
    ...
    async _do_options_deps( run_context, deps_domain, cancel ) {
        ...
        const promises = deps.map( ( id ) => {
            ...
            return run_context.get_promise( id ).promise;
        } );
