...
class Block {
        ...
        try {
            result = await ( async () => {
                ...
                if ( cache && options_key ) {
                    if ( typeof options_key === 'string' ) {
                        key = this._options.key;

                    } else if ( typeof options_key === 'function' ) {
    ...
    */
    ...
    //  FIXME: Должна ли тут прерываться цепочка, если options.error что-то вернул / бросил эксепшен?
    //
