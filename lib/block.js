    ...
    async _run( cancel, params, context ) {
        ...
        try {
            ...
            const result = await ( async () => {
                ...
                //  cancel.throw_if_cancelled();
                if ( result !== undefined ) {
                    //  FIXME: Неплохо бы deps передавать вообще везде, наравне с result.
                ...
                if ( cache && this._options.key ) {
                    key = this._options.key( { params, context } );
                    if ( key ) {
                ...
                //  FIXME: Может сделать отдельный цикл after/error?
                //  Там может быть несколько итераций же?
                result = await this._do_options_after( cancel, params, context, result );
                cancel.throw_if_cancelled();

                //  FIXME: Не должно ли это быть до after? Сразу после action?
            ...
            //  FIXME.
            //  FIXME: А не нужно ли это делать до reject?
            //  И если у нас получилась не ошибка, то нужно resolve делать?
