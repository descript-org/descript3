...
class Block {
    ...
    async _run( cancel, params, context ) {
        ...
        try {
            result = await ( async () => {
                deps = await this._do_options_deps( cancel, context );
                cancel.throw_if_cancelled();

                params = this._get_params( params, context, deps );

                //  FIXME: А не выпилить ли guard вообще в пользу before?
                //  Видимо, нет, сложно понять, что вернуть из before, чтобы не выполнять блок.
                //  Нужна какая-то специальная ошибка, проще в гарде.
                ...
                try {
                    result = await this._action( cancel, params, context, deps );
    ...
    _do_options_deps( cancel, context ) {
    ...
    async _do_options_after( cancel, params, context, deps, result ) {
        ...
        if ( option_after ) {
            for ( let i = 0; i < option_after.length; i++ ) {
                const after_result = await option_after[ i ]( { params, context, deps, result } );
                cancel.throw_if_cancelled();
    ...
    _do_options_error( cancel, params, context, deps, error ) {
        ...
        if ( option_error ) {
            for ( let i = 0; i < option_error.length; i++ ) {
                try {
                    const result = option_error[ i ]( { params, context, deps, error } );
                    if ( result !== undefined ) {
                        return result;
                    }
