...
class Block {
    ...
    async _do_options_deps( run_context, block_cancel, deps_domain, n_parents ) {
        ...
        run_context.waiting_for_deps.push( {
            //  FIXME: А точно тут block_cancel уникальный?
        ...
        try {
            ...
            const index = run_context.waiting_for_deps.findIndex( ( item ) => item.block_cancel === block_cancel );
            if ( index > -1 ) {
                run_context.waiting_for_deps.splice( index, 1 );
            }
        }
    }
