...
class Block {
    ...
    async _run( { run_context, block_cancel, deps_domain, cancel, params, context, prev } ) {
        let h_timeout = null;
        ...
        try {
            ...
            if ( !lifecycle || !lifecycle.length ) {
                ...
                //
                result = await this._do_action( run_context, block_cancel, deps_domain, cancel, params, context, deps );

            } else {
                result = await this._do_lifecycle_step( 0, run_context, block_cancel, deps_domain, cancel, params, context, deps );
            }
    ...
    async _do_lifecycle_step( index, run_context, block_cancel, deps_domain, cancel, params, context, deps ) {
        const lifecycle = this._options.lifecycle;
        ...
        try {
            ...
            if ( result === undefined ) {
                if ( index < lifecycle.length - 1 ) {
                    result = await this._do_lifecycle_step( index + 1, run_context, block_cancel, deps_domain, cancel, params, context, deps );
    ...
    async _do_action( run_context, block_cancel, deps_domain, cancel, params, context, deps ) {
        let result;
