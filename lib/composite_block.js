...
class CompositeBlock extends Block {
    ...
    async _run_groups( cancel, params, context ) {
        ...
        for ( let i = 0; i < groups.length; i++ ) {
            ...
            const promises = group.map( ( item ) => {
                const item_cancel = composite_cancel.create();
                const promise = item.block._run( item_cancel, params, context );
    ...
    block_action_started_in_context( context ) {
        //  Do nothing.
        //  console.log( 'composite.block_action_started_in_context', context._n_blocks, context._n_active_blocks );
    }

    block_action_stopped_in_context( context ) {
        //  Do nothing.
        //  console.log( 'composite.block_action_stopped_in_context', context._n_blocks, context._n_active_blocks );
    }
}
