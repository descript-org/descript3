...
async function request( options, context, cancel ) {
    const request_options = new RequestOptions( options, context, cancel );
    ...
    while ( true ) {
        ...
        try {
            ...
            if ( retries < request_options.max_retries && request_options.is_retry_allowed( error, request_options ) ) {
                context.log( {
                    type: Logger.EVENT.REQUEST_ERROR,
