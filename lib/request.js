...
const DEFAULT_OPTIONS = {
    ...
    is_retry_allowed: function( error, http_options ) {
        if ( error.id === ERROR_ID.TCP_CONNECTION_TIMEOUT ) {
            return true;
        }

        const method = http_options.method;
...
async function request( options, context, cancel ) {
    ...
    while ( true ) {
        ...
        try {
            ...
            if ( retries < options.max_retries && options.is_retry_allowed( error, error.request_options ) ) {
                context.log( {
                    type: Logger.EVENT.REQUEST_ERROR,
                ...
                if ( options.retry_timeout > 0 ) {
                    await wait_for( options.retry_timeout );
                }
