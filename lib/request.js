...
const { decompress } = require( '@fengkx/zstd-napi' );
...
class RequestOptions {
    ...
    constructor( options ) {
        ...
        this.body_compress = options.body_compress;
        ...
        if ( options.body && ( method === 'POST' || method === 'PUT' || method === 'PATCH' || method === 'DELETE' ) ) {
            ...
            if ( this.body_compress ) {
                this.http_options.headers[ 'content-encoding' ] = 'gzip';
...
class Request {
    ...
    start() {
        ...
        try {
            ...
            if ( this.options.body_compress && this.options.body ) {
                const gzip_stream = createGzip( this.options.body_compress );
                gzip_stream.pipe( this.req, { end: true } );
                gzip_stream.end( this.options.body, () => {
    ...
    async request_handler( res ) {
        ...
        const unzipped = decompress_response( res );
        ...
        try {
            ...
            const is_aborted = error.code === 'ECONNRESET';
            if ( !is_aborted ) {
                throw error;
            }
        }
...
class ZstdDecompress extends Transform {
    ...
    async _flush( callback ) {
        try {
            this.push(
                await decompress( Buffer.concat( this._receivedChunks, this._receivedLength ) ),
            );
            callback();
        } catch ( e ) {
            callback( e );
...
function decompress_response( res ) {
